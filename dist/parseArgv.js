...
module.exports = function parseArgv(argv) {
  ...
  if (argv.ignore) {
    options.patterns = options.patterns.concat(argv.ignore.split(",").map(dir => `!${dir}`));
  }
  ...
  if (argv.getLoc) {
    options.getLoc = vm.runInNewContext(argv.getLoc, makeModuleEnv(path.join(process.cwd(), "grep-ast.js")));
  }
