...
(async function main() {
  try {
    ...
    const patternsSpinner = ora(chalk.blue(`Matching patterns: ${JSON.stringify(options.patterns || defaults.patterns)}`) + chalk.grey(` (gitignore: ${options.gitignore || defaults.gitignore ? "enabled" : "disabled"})`));
    patternsSpinner.start();
    const filesSpinner = ora();
    const results = await grepAst(argv, files => {
    ...
    results.forEach(result => {
      if (result.error) {
        ...
        const codeFrame = babelCodeFrame.codeFrameColumns(result.contents, {
          start: {
        ...
        process.stderr.write(result.filepath + chalk.grey(`:${result.loc.start.line}:${result.loc.start.column}-${result.loc.end.line}:${result.loc.end.column}`) + "\n");
    ...
    nonErrorResults.forEach(result => {
      ...
      process.stdout.write(result.filepath + chalk.grey(`:${result.loc.start.line}:${result.loc.start.column}-${result.loc.end.line}:${result.loc.end.column}`) + "\n");
    });
  } catch (error) {
